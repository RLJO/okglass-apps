                ...
                if response.status_code == 404:
                    ...
                    if object_antsroute:
                        if object_antsroute and 'status' in object_antsroute and object_antsroute['status']:
                            ...
                            _logger.warning("ERROR Time : %s Code : %s Message : %s" % (object_antsroute['timestamp'], object_antsroute['status'], object_antsroute['error']))
                            vals_log.update({
                                'date_log': datetime.now(),
                                'log_type': 'error',
    ...
    def antsroute_send_request(self, pickings):
        for carrier in self:
            ...
            if self.antsroute_destination:
                ...
                raise UserError(_('API Antsroute : No destination defined on delivery carrier %s.' % carrier.name))
            ...
            for pick in pickings:
                ...
                if exist:
                    ...
                    if delete:
                        _logger.warning("API Antsroute : Previous Pickup deleted")
                ...
                if object_antsroute:
                    if object_antsroute and 'status' in object_antsroute and object_antsroute['status']:
                        if object_antsroute['status'] == 201:
                            pick.write({'sent_to_carrier': send_datetime})
                        else:
                            # Error in API we log the details
                            _logger.warning("ERROR Time : %s Code : %s Message : %s" % (object_antsroute['timestamp'], object_antsroute['status'], object_antsroute['messages']))
                            vals_log.update({
                                'date_log':datetime.now(),
                                'log_type':'error',
                                'name':_("API Antsroute : ERROR [%s] %s") % (response.status_code, object_antsroute['messages']),
                            })
                            delivery_log = self.env['delivery.carrier.log'].create(vals_log)
                    else:
                        # All good
                        if response.status_code != 201:
                            ...
                            pick.update({'sent_to_carrier': send_datetime})
                            vals_log.update({
                                'date_log':datetime.now(),
                                'log_type':'info',
                                'name':_("API Antsroute : Pickup created successfully"),
                            })
                            delivery_log = self.env['delivery.carrier.log'].create(vals_log)
                else:
                    vals_log.update({
                        'date_log':datetime.now(),
                        'log_type':'error',
                        'name':_("API Antsroute : Antsroute did not answer, please try again later"),
                    })
                    delivery_log = self.env['delivery.carrier.log'].create(vals_log)
