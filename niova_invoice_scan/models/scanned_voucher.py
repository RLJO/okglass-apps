...
class ScannedVoucher(models.Model):
    ...
    def _compute_currency(self):
        for voucher in self:
            ...
            # Check module default currency
            currency = self._default_currency(currency)
            if currency:
                voucher.currency_id = currency.id
                voucher.default_currency = False
            else:
                # System global default currency
                voucher.currency_id = self.env['account.move'].with_context(default_move_type='in_invoice')._get_default_currency().id
                voucher.default_currency = True
    
    def _default_currency(self, currency):
        # Exception for USD
        if currency and currency.symbol == '$':
            # If there is a default currency then check that
            get_param = self.env['ir.config_parameter'].sudo().get_param
            default_currency_id = get_param('invoice_scan_currency', False)
            if default_currency_id:
                default_currency_id = int(default_currency_id)
                if default_currency_id != currency.id:
                    default_currency = self.env['res.currency'].browse(default_currency_id)
    ...
    def action_open_invoice(self):
        if self.invoice_id:
            ...
            if self.voucher_type == 'in_refund':
                ctx = {'default_move_type': 'in_refund', 'move_type': 'in_refund', 'journal_type': 'purchase'}
            else:
                ctx = {'default_move_type': 'in_invoice', 'move_type': 'in_invoice', 'journal_type': 'purchase'}
